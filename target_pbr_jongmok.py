...
def execute(qry=run_qry):
    ...
    df_base = df_base.rename(columns={"PERFORM_VAL": "JONGMOK_PBR"})
    ...
    for idx, row in df_jongmok_list.iterrows():
        ...
        df_jongmok = df_jongmok[df_jongmok["YEAR_MM"] == df_jongmok.YEAR_MM.max()][["JONGMOK_NM", "JONGMOK_PBR"]]
        list_result.append([row.JONGMOK_NM, df_jongmok.JONGMOK_PBR.max()])
    
    df_result = pd.DataFrame(list_result, columns=["JONGMOK_NM", "JONGMOK_PBR"])
